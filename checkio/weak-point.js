...
function weakPoint(grid) {
    var colSum = [];
    var rowSum = grid.reduce(function (acc, currVal) {
        currVal.forEach(function (val, i) {
            acc[i] = (acc[i] || 0) + val;
        });
        return acc;
    }, []);
    ...
    grid.forEach(function (val) {
        colSum.push(val.reduce((acc, currVal) => acc + currVal));
    });

    function indexOfSmallest(a) {
        wP.push(a.indexOf(Math.min.apply(Math, a)));
    }

    indexOfSmallest(colSum);
    indexOfSmallest(rowSum);
